import {
	  ...
	  subcommand
		...
		.addBooleanOption((option) =>
		  option
			...
			.setDescriptionLocalization("tr", "İlanın perakende olup olmadığı")
			.setRequired(true)
		)
	)
	.addSubcommand((subcommand) =>
		...
		add: {
		  execute: async (interaction: AutocompleteInteraction) => {
			...
			if (!user) {
			  ...
			  //   .filter(({ loot }) => loot.title.toLowerCase().includes(value))
			  //   .forEach(({ loot }) => console.log(loot));
			...
			return await interaction.respond(
			  user.inventory.items
				...
				.map(({ item }) => ({
				  name: item.title,
				  value: `item.${(item as any)._id.toString()}`,
				}))
				.concat(
				  user.inventory.loots
					.filter(({ loot }) =>
					...
					.map(({ loot }) => ({
					  name: loot.title,
					  value: `loot.${(loot as any)._id.toString()}`,
					}))
				)
			);
		  },
  ...
  //   console.log(value);
	...
	const item =
	  value[0] === "item"
		? user.inventory.items.find(({ item }) => (item as any)._id.toString() === value[1])
		: null;
	const loot = !item
	  ? user.inventory.loots.find(({ loot }) => (loot as any)._id.toString() === value[1])
	  : null;
	console.log(item, loot);
	...
	const itemID = (
	  item
		? ((await Item.findOne({ _id: (item.item as any)._id.toString() }).exec()) as any)
		: ((await Loot.findOne({ _id: (loot!.loot as any)._id.toString() }).exec()) as any)
	)?._id.toString();
	console.log(itemID);
	  ...
	  embeds: [
		interaction.client.embedManager.Success({
		  content: getTranslation(
			...
			interaction.locale,
			{
			  amount: (String(amount) + "x").replace("$item", thing.item.title),
			  price: String(price),
			  item: thing.item.title,
			}
		  ).replace("$itemEmoji", thing.item.emoji || ""),
  ...
  async function remove(
	...
	if (remaining) {
	  const inv =
		user.inventory.items.find(({ item }) => item.id === listing.itemID) ||
  ...
  async function update(
	...
	const modal = new ModalBuilder()
	  .setTitle(getTranslation("commands", "market.update", interaction.locale))
