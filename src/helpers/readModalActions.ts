...
export default async function readModalActions(path: string): Promise<ModalActionsReadResult> {
    ...
    const commandReadResult: ModalActionsReadResult = {
        success: true,
        modals: []
    };
    ...
    for (const modal of modals) {
        ...
        if (!commandFile.name || !commandFile.execute) return {
            success: false,
            error: `The modal action listener file "${modal}" does not contain a name or execute function.`
        };

        // Add the validated modal to the result object.
        commandReadResult.modals.push({
